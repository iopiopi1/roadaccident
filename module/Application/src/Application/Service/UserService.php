...
class UserService extends EntityServiceAbstract {
	...
	public function sendConfirmationEmail($email,$username,$id,$password,$server_url){
		...
		$recipient = $email;
		$subject="Регистрация на cardam.ru";
		$headers = "MIME-Version: 1.0\r\n";
		$headers .= "Content-Type: text/html; charset=UTF-8\r\n";
		$headers .= 'From: admin@cardam.ru ';
		$mail_body = '
						  ...
						  <table class="layout layout--no-gutter" style="border-collapse: collapse;table-layout: fixed;Margin-left: auto;Margin-right: auto;overflow-wrap: break-word;word-wrap: break-word;word-break: break-word;background-color: #31ace0;" align="center">
							<tbody><tr>
							  <td class="column" style="padding: 0;text-align: left;vertical-align: top;color: #565656;font-size: 14px;line-height: 21px;font-family: Georgia,serif;" width="300">
								...
								<div style="Margin-left: 20px;Margin-right: 20px;Margin-top: 24px;Margin-bottom: 24px;">
						  <p style="Margin-top: 0;Margin-bottom: 0;"><strong><span style="color:#fbfbfb">cardam.ru</span></strong></p><p style="Margin-top: 20px;Margin-bottom: 0;">cardam.ru</p>
						</div>
		...
		$email->setExtraHeader('-f admin@cardam.ru');
	...
	public function sendRestoredDetailsEmail($email,$username,$id,$password,$server_url){
		$hash_string = md5($email . $username . $id . $password);
		$link = $server_url . dirname($_SERVER['PHP_SELF']) . 'api/restorepassconfirm/' . $hash_string;
		
		$recipient = $email;
		$subject="Восстановление пароля на cardam.ru";
		$headers = "MIME-Version: 1.0\r\n";
		$headers .= "Content-Type: text/html; charset=UTF-8\r\n";
		$headers .= 'From: admin@cardam.ru ';
		$mail_body = '
					...
					<h1 style="Margin-top: 0;Margin-bottom: 0;font-style: normal;font-weight: normal;color: #565656;font-size: 26px;line-height: 34px;font-family: Avenir,sans-serif;">
					<span style="color:#fbfbfb">Здравствуйте, ' . $username . '. </span></h1><p style="Margin-top: 20px;Margin-bottom: 0;">
					<span style="color:#fbfbfb">Вы запросили восстановление пароля на нашем сервисе. Ваш новый пароль: ' . $password . '. Для восстановления пароля Вашего аккаунта пройдите по ссылке:</span></p>
	...
	public function sendInfoDeletionImageEmail($userId, $reason, $server_url, $imagePath){
		$link = $server_url . dirname($_SERVER['PHP_SELF']) . 'index';
		$linkImgTmpPath = $server_url . dirname($_SERVER['PHP_SELF']) . $imagePath;
		$user = $this->getEntityManager()->getRepository('\Application\Entity\User')->findOneBy(array('id' => $userId));
		$email = $user->getEmail();
		$username = $user->getUsername();
		$recipient = $email;
		$subject="Модерация фото, добавленных Вами на cardam.ru";
		$headers = "MIME-Version: 1.0\r\n";
		$headers .= "Content-Type: text/html; charset=UTF-8\r\n";
		$headers .= 'From: admin@cardam.ru ';
		$mail_body = '
					...
					<h1 style="Margin-top: 0;Margin-bottom: 0;font-style: normal;font-weight: normal;color: #565656;font-size: 26px;line-height: 34px;font-family: Avenir,sans-serif;">
					<span style="color:#fbfbfb">Здравствуйте, ' . $username . '. </span></h1><p style="Margin-top: 20px;Margin-bottom: 0;">
					<span style="color:#fbfbfb">К сожалению, фотография,добавленная Вами, не прошли регистрацию. Причина</span></p>
					<p style="Margin-top: 20px;Margin-bottom: 0;">&nbsp;</p>
					<p>' . $reason . '</p>
					<img src="' . $linkImgTmpPath . '"></img><br/>
					<a href="' . $link . '">' . $link . '</a><p style="Margin-top: 20px;Margin-bottom: 0;">&nbsp;</p>
						</div>
	...
	public function sendInfoApprovedImageEmail($userId, $server_url, $imagePath, $vehiclePath){
		$link = $server_url . dirname($_SERVER['PHP_SELF']) . 'index';
		$linkImgTmpPath = $server_url . dirname($_SERVER['PHP_SELF']) . $imagePath;
		$linkVehicle = $server_url . dirname($_SERVER['PHP_SELF']) . $vehiclePath;
		...
		$mail_body = '
					...
					<h1 style="Margin-top: 0;Margin-bottom: 0;font-style: normal;font-weight: normal;color: #565656;font-size: 26px;line-height: 34px;font-family: Avenir,sans-serif;">
					<span style="color:#fbfbfb">Здравствуйте, ' . $username . '. </span></h1><p style="Margin-top: 20px;Margin-bottom: 0;">
					<span style="color:#fbfbfb">Фотография,добавленная Вами успешно прошла модерацию.</span></p>
					<p style="Margin-top: 20px;Margin-bottom: 0;">&nbsp;</p>
					<a href="' . $linkVehicle . '"><img src="' . $linkImgTmpPath . '"></img></a><br/>
					<a href="' . $link . '">' . $link . '</a><p style="Margin-top: 20px;Margin-bottom: 0;">&nbsp;</p>
						</div>
	...
	public function sendInfoUserUpdateEmail($userId, $reason, $server_url){
		$user = $this->getEntityManager()->getRepository('\Application\Entity\User')->findOneBy(array('id' => $userId));
		$email = $user->getEmail();
		$username = $user->getUsername();
		$recipient = $email;
		$subject="Изменение статуса вашего аккаунта на cardam.ru";
		$headers = "MIME-Version: 1.0\r\n";
		$headers .= "Content-Type: text/html; charset=UTF-8\r\n";
		$headers .= 'From: admin@cardam.ru ';
		$mail_body = '
					...
					<h1 style="Margin-top: 0;Margin-bottom: 0;font-style: normal;font-weight: normal;color: #565656;font-size: 26px;line-height: 34px;font-family: Avenir,sans-serif;">
					<span style="color:#fbfbfb">Здравствуйте, ' . $username . '. </span></h1><p style="Margin-top: 20px;Margin-bottom: 0;">
					<span style="color:#fbfbfb">К сожалению, ваш аккаунт был заблокирован.</span></p>
					<p style="Margin-top: 20px;Margin-bottom: 0;">&nbsp;</p>
					<p> Причина: ' . $reason . '</p>
					<p style="Margin-top: 20px;Margin-bottom: 0;">&nbsp;</p>
						</div>
    ...
    public function checkLogin($username,$password)
    {
		...
		array(
			'username' => $username, 
			'password' => $password,
			'status' => \Application\Entity\User::STATUS_ACTIVE,
			)
		);
	...
	public function getIsUserAdmin($userId){
		...
		if(!is_null($admin) && !is_null($admin->getId())){
			$isAdmin = 1;
		}
		else{
			$isAdmin = 0;
